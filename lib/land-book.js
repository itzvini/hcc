...
async function sweep() {
  ...
  for (const r of rows) {
    const local = Number(r.end_time) <= now ? 'expired'
      ...
      : null;
    if (local) { await db.closeLandOrder(r.order_hash, 'invalid', local); closed++; continue; }
    live.push(r);
  }
  ...
  for (const r of live) {
    ...
    if (verdict) { await db.closeLandOrder(r.order_hash, verdict[0], verdict[1]); closed++; continue; }
...
async function prepareBuy({ orderHash, taker }) {
  ...
  const now = Math.floor(Date.now() / 1000);
  if (Number(row.end_time) <= now) { await db.closeLandOrder(orderHash, 'invalid', 'expired'); invalidate(); fail('not_active', 409); }
  ...
  const dead = owner !== String(row.offerer).toLowerCase() ? ['invalid', 'owner_changed']
    ...
    : null;
  if (dead) { await db.closeLandOrder(orderHash, dead[0], dead[1]); invalidate(); fail('not_active', 409); }
