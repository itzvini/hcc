...
const ETH_RPC_URL   = (process.env.ETH_RPC_URL || 'https://eth.blockscout.com/api/eth-rpc').trim();
...
// Unsigned on-chain Seaport cancel for ONE of the caller's own listings. Found by hash in
// the collection feed (which carries full parameters); the offerer must match the caller,
// and Seaport itself only lets the offerer cancel. On-chain (costs gas) so the order is
// truly dead — an off-chain OpenSea hide would leave the signature fillable until expiry.
async function prepareCancel({ orderHash, maker }) {
  if (!configured()) fail('not_configured', 503);
  const addr = String(maker).toLowerCase();
  const raw = await scanRawListings();
  const found = raw.find(l => String(l.order_hash).toLowerCase() === String(orderHash).toLowerCase());
...
module.exports = {
  configured, sellEnabled, listListings, getToken, ownedLand, prepareBuy, allParcelCoords,
  prepareListing, createListing, myListings, prepareCancel, LAND_CONTRACT,
};
