...
// parcel into), best first. Settled in WETH: the bidder's `offer` is the WETH they pay;
// the `consideration` carries the wanted NFT (criteria) plus WETH fee items, so the seller
// nets offer − fees. Shaped to mirror the Creature offer rows the client already renders.
async function listCollectionOffers() {
  ...
  const offers = (body.offers || []).map(o => {
    ...
    const gross = BigInt(offerItem.startAmount || '0');
    ...
    const net = gross > feeWei ? gross - feeWei : gross;
    return {
      ...
      priceEth: wei2eth(gross),    // what the bidder pays (WETH ≈ ETH)
      netEth: wei2eth(net),        // what a seller nets after fees
      grossWei: gross.toString(),
      expiresAt: p?.endTime ? Number(p.endTime) : null,
    };
  }).filter(Boolean);
  offers.sort((a, b) => b.priceEth - a.priceEth); // best first
  return { offers };
}
