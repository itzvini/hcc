...
// What OpenSea will actually SETTLE on Ethereum is narrower than the tables here, and narrower
// than Seaport itself allows. Measured against the live API on 2026-09-07 by posting orders in
// each token: listings take native ETH + WETH, offers take WETH. USDC, DAI and USDT are all
// refused, for BOTH order types:
//   400 "Payment asset 0xa0b8…eb48 is not supported for listings on this chain"
//   400 "Payment asset 0xa0b8…eb48 is not supported for offers on this chain"
// It is not a collection setting: every Ethereum and Base collection sampled reports the same
// ETH/WETH pair, and no dollar token passes on any of them. (Polygon is where USDC listings
// exist — different chain allowlist.)
//
// The rejection lands at CREATE, after the user has already committed: a seller has signed,
// and a USDC BIDDER has additionally paid mainnet gas for a conduit approval. So both paths
// died with real money and a wallet signature spent on an order that could never exist (a
// holder hit the listing half in prod 2026-09-07). Refuse it up front instead.
//
// These are CREATE allowlists only. Reading stays wide (landCurrencyBySymbol,
// landCurrencyByOfferToken) so an order in another token would still display, not crash.
const LAND_LISTING_CURRENCIES = { eth: LAND_CURRENCIES.eth };
const landListingCurrency = key => LAND_LISTING_CURRENCIES[String(key || '').toLowerCase()] || null;
...
// ...but only WETH can actually be CREATED (see the note above LAND_LISTING_CURRENCIES).
const LAND_OFFER_CREATE_CURRENCIES = { eth: LAND_OFFER_CURRENCIES.eth };
const landOfferCreateCurrency = key => LAND_OFFER_CREATE_CURRENCIES[String(key || '').toLowerCase()] || null;
...
// OpenSea's SignedZone (SIP-7). An order pointing at it can only be filled with a
// server-issued signature OpenSea puts in `extraData`, which is what makes their off-chain
// cancellation binding rather than cosmetic: refuse to sign and the order is dead.
//
// MEASURED 2026-09-10, live, on this collection and four majors (BAYC, Pudgy, Azuki,
// Doodles, MAYC): every one of 248 sampled LISTINGS uses zone 0x0 / orderType 0, and not
// one uses this zone. Every LAND OFFER does use it (orderType 2 and 3), including the ones
// OpenSea builds for us, and their fulfilment endpoint still issues a 126-byte extraData
// signature for them. So off-chain cancellation is real and safe for offers, and simply
// does not arise for listings — for a zone-0 order it would only de-index the order while
// the signature stayed fillable straight on Seaport. cancelMode() below routes on that,
// per order, rather than on a guess about which kind of order this is.
...
/**
 * Is this OpenSea transfer event a mint? `transfer_type: 'mint'` is the authoritative signal
 * and the only one that catches recent LAND withdrawals; the zero-address `from` is the older
 * shape and still appears on pre-2026 events, so both are checked.
 */
function isMintEvent(e, from) {
  return String(e?.transfer_type || '').toLowerCase() === 'mint'
    || String(from ?? e?.from_address ?? '').toLowerCase() === ZERO_ADDR;
}
...
// Which book(s) a new listing goes into:
//   'opensea' — relayed to OpenSea. Carries their 1%, ETH only, seen by every aggregator.
//   'book'    — kept here (lib/land-book.js). Creator royalty only, ETH or USDC, ours alone.
//   'both'    — one signed order of each, so the parcel keeps OpenSea's reach and is also
//               fillable here a percent cheaper. Two signatures, two independent orders.
// Own kill-switch: LAND_BOOK=0 takes the house book out without touching the OpenSea path.
const LISTING_DESTINATIONS = new Set(['opensea', 'book', 'both']);
const bookEnabled = () => process.env.LAND_BOOK !== '0';
...
// `required` is OpenSea's own word for it, and it means required BY OPENSEA: their 1% is the
// price of using their book, the creator royalty is not. That distinction is what the house
// book is built on — an order we keep ourselves owes the creator royalty and nothing else.
const DEFAULT_FEES = [
  { bps: 100, recipient: '0x0000a26b00c1f0df003000390027140000faa719', required: true },  // OpenSea 1%
  { bps: 500, recipient: '0xc4862a6e1c1552bce246e3dc7e7fc0f7bc647bfb', required: false }, // creator 5% (PW royalty wallet)
];

let ethers = null;
try { ethers = require('ethers'); } catch { /* transitive dep — present in practice */ }
const crypto = require('crypto');
const { assetTransfers, alchemyEnabled } = require('./eth-rpc');
...
const seaportIface = () => new ethers.Interface([
  ...
  // Plain, whole-order fulfilment. OpenSea never hands us this one (their endpoint returns
  // the gas-optimized basic variant), but our own book builds its buys from the stored order
  // rather than from a remote answer, and a one-of-one ERC-721 sale is exactly what this
  // takes. Simpler to encode than the basic variant and it cannot mis-shape: the parameters
  // ARE the signed order. See lib/land-book.js.
  `function fulfillOrder((${ORDER_PARAMETERS} parameters, bytes signature) order, bytes32 fulfillerConduitKey) payable returns (bool fulfilled)`,
...
const fail = (code, statusCode, logMsg) => {
  ...
  // `log` rides along so a caller can tell one 503 from another — the off-chain cancel has
  // to distinguish "OpenSea refused this signature" from "OpenSea is down". Never sent to a
  // client: callers map it to a code first (see submitOffchainCancel).
  throw Object.assign(new Error(code), { code, statusCode, log: logMsg || '' });
};
...
async function listListings(cursor) {
  ...
  const items = (body.listings || []).map(l => {
    ...
    return {
      ...
      // Which book holds this order. The buy path routes on it, and the grid says so out
      // loud, because "where is this listed" changes what the buyer pays (see lib/land-book.js).
      source: 'opensea',
      ...
      // When the seller put it up. Seaport stamps it in whole seconds. A price guide needs
      // this to tell a plot nobody wants from a plot nobody is selling: the first has been
      // sitting on the shelf for months, the second was never on it.
      listedAt: Number(p?.startTime) > 0 ? Number(p.startTime) * 1000 : 0,
      name: `Highrise LAND #${tokenId}`,
      image: null,
    };
  }).filter(Boolean)
    // Stolen treasury parcels never reach the buy grid. Filtered at the source so no
    // downstream consumer (browse, price guide, floor) has to know about the blocklist.
    .filter(it => !stolen.isBlocked({ tokenId: it.tokenId, wallet: it.seller }));
...
async function listCollectionOffers() {
  ...
  const offers = (body.offers || []).map(o => {
    ...
    return {
      ...
      freeCancel: cancelMode(p) === 'offchain',
...
async function getToken(tokenId) {
  ...
  // Flagged, not hidden: a 404 here would look like a broken link and would also hide the
  // parcel from anyone checking WHY it can't be traded. The client turns this into a notice
  // in place of the Buy button.
  const blocked = stolen.isBlocked({ tokenId, wallet: owner });
  return {
    tokenId: String(tokenId), owner: owner || null, ...meta,
    ...(blocked ? { blocked: true, blockedReason: stolen.BLOCK_NOTICE } : {}),
  };
}
...
async function ownedLand(address) {
  ...
  do {
    ...
    for (const n of (body.nfts || [])) {
      ...
      const tokenId = String(n.identifier);
      items.push({
        tokenId,
        name,
        image: n.display_image_url || n.image_url || null,
        coords: landCoords(name, null), // account listings carry no traits — name only
        // Flagged so the Sell grid can grey the row out. The server refuses it again in
        // prepareListing / prepareAcceptOffer — this is the courtesy, not the guard.
        ...(stolen.isBlocked({ tokenId, wallet: address }) ? { blocked: true } : {}),
      });
    }
    next = body.next || null;
    pages++;
...
async function prepareBuy({ orderHash, protocolAddress, taker }) {
  ...
  // The blocklist check has to happen HERE, not on the way in: an order hash doesn't name
  // the parcel or the seller, so we need the order OpenSea just returned. Refusing after the
  // fetch costs one wasted call and is the only way to be sure.
  const orderParams = body.fulfillment_data?.orders?.[0]?.parameters;
  if (stolen.isBlocked({
    tokenId: orderParams?.offer?.[0]?.identifierOrCriteria,
    wallet: orderParams?.offerer,
  })) fail('blocked_stolen', 451, 'stolen treasury parcel — refusing to broker');
...
async function prepareAcceptOffer({ orderHash, protocolAddress, tokenId, taker }) {
  if (!configured()) fail('not_configured', 503);
  if (stolen.isBlocked({ tokenId, wallet: taker })) fail('blocked_stolen', 451, 'stolen treasury parcel — refusing to broker');
...
async function getFees() {
  ...
  try {
    ...
    const fees = (col.fees || [])
      .map(f => ({ bps: Math.round(Number(f.fee) * 100), recipient: String(f.recipient || '').toLowerCase(), required: !!f.required }))
...
/**
 * Build ONE Seaport listing over `price`, splitting it into the seller's proceeds and the
 * given fee schedule. The fee list is the only thing that differs between an OpenSea order
 * and a house order, so both come out of here and nothing about the order shape can drift
 * between them. Each order gets its own salt: two orders for the same parcel at the same
 * price must be two distinct, independently fillable offers.
 */
function buildListingOrder({ target, fees, cur, price, tokenId, maker, counter, startTime, endTime }) {
  let feeTotal = 0n;
  ...
  const consideration = [
    { itemType: cur.itemType, token: cur.token, identifierOrCriteria: '0', startAmount: proceeds.toString(), endAmount: proceeds.toString(), recipient: maker },
    ...feeItems,
  ];
  ...
  const message = {
    offerer: maker, zone: ZERO_ADDR, offer, consideration,
    orderType: 0, startTime: String(startTime), endTime: String(endTime),
    zoneHash: ZERO_HASH, salt: '0x' + crypto.randomBytes(32).toString('hex'), conduitKey: CONDUIT_KEY, counter,
  };
  ...
  return {
    target, message, orderParameters, currency: cur.code, counter: String(counter),
    orderHash: ethers.TypedDataEncoder.hashStruct('OrderComponents', SEAPORT_TYPES, message),
    priceUnits: price.toString(), proceedsUnits: proceeds.toString(), feeUnits: feeTotal.toString(),
    ...money,
  };
}

async function prepareListing({ tokenId, currency = 'eth', priceUnits, maker, durationDays, destination = 'opensea' }) {
  if (!configured()) fail('not_configured', 503);
  if (!sellEnabled()) fail('disabled', 503);
  if (stolen.isBlocked({ tokenId, wallet: maker })) fail('blocked_stolen', 451, 'stolen treasury parcel — refusing to broker');
  const want = LISTING_DESTINATIONS.has(destination) ? destination : 'opensea';
  const cur = landCurrency(currency);
  if (!cur) fail('bad_currency', 400);

  // Which books can actually take THIS order. OpenSea settles listings in native ETH only,
  // so a USDC price can go in our book and nowhere else. Rather than refusing a sale we can
  // perfectly well make, drop the book that can't take it and report the drop — the seller
  // finds out before they sign, not after (see LAND_LISTING_CURRENCIES for the 7 Sep 2026
  // case where they found out after).
  const skipped = [];
  let targets = want === 'both' ? ['opensea', 'book'] : [want];
  if (targets.includes('opensea') && !landListingCurrency(cur.code)) {
    targets = targets.filter(x => x !== 'opensea');
    skipped.push({ target: 'opensea', reason: 'currency_unsupported' });
  }
  if (targets.includes('book') && !bookEnabled()) {
    targets = targets.filter(x => x !== 'book');
    skipped.push({ target: 'book', reason: 'disabled' });
  }
  // Nothing left to sign. Name the reason the last book dropped out rather than a bare
  // "unavailable" — it is the difference between "pick another currency" and "come back later".
  if (!targets.length) fail(skipped[0]?.reason || 'currency_unsupported', 400);

  const owner = await readOwnerOf(tokenId);
  if (owner == null) fail('not_found', 404);
  const me = maker.toLowerCase();
  if (owner !== me) fail('not_owner', 400);

  // Round down to a clean multiple so every fee split stays an exact integer. 10000 units is
  // dust in ETH (18 dec) and 1 cent in USDC (6 dec) — fine either way.
  const price = BigInt(priceUnits) - (BigInt(priceUnits) % 10000n);
  if (price <= 0n) fail('bad_price', 400);

  const fees = await getFees();
  // An order kept here owes the creator royalty and nothing else: OpenSea's cut is the price
  // of OpenSea's book, and `required` is their own flag for exactly that (see DEFAULT_FEES).
  const houseFees = fees.filter(f => !f.required);
  const counter = (await readCounter(me)).toString();
  const now = Math.floor(Date.now() / 1000);
  const days = Math.min(30, Math.max(1, Math.floor(Number(durationDays) || 7)));
  const common = { cur, price, tokenId, maker: me, counter, startTime: now, endTime: now + days * 86400 };
  const orders = targets.map(target => buildListingOrder({
    target, fees: target === 'book' ? houseFees : fees, ...common,
  }));

  // One approval covers every order here: they all route through the same conduit, and the
  // seller pays for it once ever, not once per book.
  const actions = [];
  if (!(await isApproved(me))) actions.push(approvalAction());
  for (const o of orders) {
    actions.push({
      type: 'SIGNABLE', purpose: 'CREATE_LISTING', target: o.target, orderHash: o.orderHash,
      typedData: { types: { EIP712Domain: EIP712_DOMAIN_FIELDS, ...SEAPORT_TYPES }, domain: seaportDomain(), primaryType: 'OrderComponents', message: o.message },
    });
  }

  const shape = ({ message, ...rest }) => rest; // the typed data already rides on the action
  const primary = orders[0];
  return {
    actions,
    orders: orders.map(shape),
    destination: orders.length > 1 ? 'both' : orders[0].target,
    skipped,
    chainId: '0x1',
    // The first order's figures, flat, so a caller that only knows about one listing (the
    // mass-list loop, anything older than the house book) keeps working unchanged.
    ...shape(primary),
  };
}
...
async function createListing({ orderParameters, signature }) {
  ...
  // The order must sell exactly one LAND parcel and be priced in ONE allowed LISTING currency —
  // every consideration item (proceeds + fees) in that same token. That means native ETH: see
  // LAND_LISTING_CURRENCIES for why USDC can't be a listing here. Never an arbitrary/worthless
  // token either way, so the June 2026 audit's "our key can't be a generic Seaport relay"
  // guarantee holds.
  const payItem = consideration.length ? landCurrencyByItem(consideration[0]?.itemType, consideration[0]?.token) : null;
  const payCur = payItem && landListingCurrency(payItem.code) ? payItem : null;
  ...
  // Re-check the blocklist on the WRITE path, not just in prepareListing. This is the call
  // that posts to OpenSea under OUR API key, and it accepts order parameters straight from
  // the client — so a caller who skips prepare would otherwise have us relay a stolen parcel
  // ourselves. Same guarantee the scope guard above exists for.
  if (stolen.isBlocked({ tokenId: offer[0]?.identifierOrCriteria, wallet: orderParameters?.offerer })) {
    fail('blocked_stolen', 451, 'stolen treasury parcel — refusing to relay listing');
  }
...
async function prepareOffer({ makerAddress, currency = 'eth', priceUnits, durationDays }) {
  ...
  const cur = landOfferCreateCurrency(currency);
  if (!cur) fail(landOfferCurrency(currency) ? 'currency_unsupported' : 'bad_currency', 400);
...
async function createOffer({ orderParameters, signature, criteria }) {
  ...
  const offer = Array.isArray(orderParameters?.offer) ? orderParameters.offer : [];
  const consideration = Array.isArray(orderParameters?.consideration) ? orderParameters.consideration : [];
  // Must be one allowed offer currency AND one OpenSea will actually create (WETH).
  const offerCur = offer.length === 1 ? landCurrencyByOfferToken(offer[0]?.itemType, offer[0]?.token) : null;
  const okOffer = !!(offerCur && landOfferCreateCurrency(offerCur.code));
...
async function myListings(address) {
  ...
  const items = raw.map(l => {
    ...
    return {
      ...
      orderHash: l.order_hash,
      protocolAddress: l.protocol_address,
      // Named even though this function only ever reads OpenSea's book: a cross-listed
      // parcel puts one of these rows beside a house row, and a row with no source would be
      // the only one on screen without a badge saying where it lives.
      source: 'opensea',
      // Free to withdraw, or does it need a paid transaction? The seller is told which
      // before they commit to a price change, so the answer travels with the row.
      freeCancel: cancelMode(p) === 'offchain',
      tokenId: String(tokenId),
      currency: cur.code,
      priceAmt: amt,
...
/**
 * One wallet's LAND events — buys, sales, transfers and mints — newest first and UNCAPPED.
 * Both `myHistory` (the dashboard timeline) and `walletTransfers` (the Sales tab's transfer
 * rows) are slices of this, so the two can never disagree about what a wallet did.
 */
async function walletEvents(address) {
  if (!configured()) fail('not_configured', 503);
  const addr = String(address).toLowerCase();
  ...
  // De-duplicate first. The account feed matches an event on EITHER side, so anything whose
  // two sides are the same address comes back twice — which is every mint, because OpenSea
  // names the recipient as both the sender and the receiver. Counting those twice turned a
  // 64-parcel withdrawal into 128 rows.
  const uniq = new Map();
  for (const e of events) {
    const key = [e.event_type, e.transaction || e.event_timestamp, e.nft?.identifier,
      (e.from_address || '').toLowerCase(), (e.to_address || '').toLowerCase()].join('|');
  ...
  const saleTxs = new Set(deduped.filter(e => e.event_type === 'sale').map(e => e.transaction).filter(Boolean));
  const entries = [];
  for (const e of deduped) {
    ...
    if (e.event_type === 'sale') {
      ...
      // A withdrawal out of the game mints the parcel, and OpenSea says so in `transfer_type`
      // — but it does NOT always set from_address to the zero address. On recent mints it
      // names the RECIPIENT on both sides, so a zero-address test alone read them as
      // "received from yourself". Trust the label first, keep the zero test for older events.
      const kind = isMintEvent(e, from) ? 'minted' : isReceiver ? 'received' : 'sent';
...
async function myHistory(address) {
  return { items: (await walletEvents(address)).slice(0, HISTORY_ITEMS_MAX) };
}

/* --- Collection-wide mints (LAND leaving the game) ---------------------------------------
   A LAND "mint" is not new supply being sold: it is a parcel the player already owned in
   Highrise being withdrawn onto Ethereum, which the LAND contract mints at that moment. The
   route is HighriseLandWithdrawal.withdraw(data, signature) and the signature is Highrise's,
   so nobody can mint a parcel for themselves. Those withdrawals were invisible here, and a
   burst of them reads on a block explorer like the team quietly printing land.

   Read from the CHAIN, not from OpenSea. OpenSea's collection feed does carry mints
   (`transfer_type: 'mint'`), but only inside the transfer stream — reaching 2022 through it
   means paging roughly 760 pages of wallet-to-wallet noise to find 3,200 mints. The chain
   answers the same question in three calls, complete and in order.

   These rows carry no image on purpose: the Sales tab renders a LAND row from its parcel
   coordinates (the slime pet), and the catalogue supplies name + coords by token id. */
const MINTS_MAX_PAGES = 5;               // 1,000 transfers a page; the whole history is ~3,200

async function collectionMints() {
  // No chain source means we cannot answer, which is NOT the same as "no parcel was ever
  // withdrawn". Returning [] here would put a confident "Withdrawals 0" on the page — the
  // false zero this codebase keeps out of every other market read.
  if (!alchemyEnabled) fail('mints_unavailable', 503);
  const rows = await assetTransfers({
    fromBlock: '0x0', toBlock: 'latest',
    contractAddresses: [LAND_CONTRACT],
    category: ['erc721'],
    fromAddress: ZERO_ADDR,              // a mint IS the transfer out of the zero address
    withMetadata: true, order: 'desc',
  }, { maxPages: MINTS_MAX_PAGES });

  const out = [];
  const seen = new Set();
  for (const t of rows) {
    const at = t?.metadata?.blockTimestamp || null;
    const tokenId = t?.tokenId != null ? BigInt(t.tokenId).toString() : null;
    if (!at || !tokenId) continue;
    const key = `${t.hash}:${tokenId}`;
    if (seen.has(key)) continue;
    seen.add(key);
    out.push({
      tokenId, event: 'mint', at, tx: t.hash || null,
      from: null, to: String(t.to || '').toLowerCase() || null,
      name: null, image: null, coords: null,
    });
  }
...
/**
 * Just the moves nobody paid for: received, sent, minted. Shaped like the Creature side's
 * transfer rows (`from`/`to` rather than `kind`/`with`) so the Sales tab speaks one language
 * across both collections. The sale legs are already gone — `walletEvents` drops the transfer
 * half of a trade by transaction hash.
 */
async function walletTransfers(address) {
  const addr = String(address).toLowerCase();
  return (await walletEvents(address))
    .filter(e => e.kind === 'received' || e.kind === 'sent' || e.kind === 'minted')
    .map(e => ({
      tokenId: e.tokenId,
      event: e.kind === 'minted' ? 'mint' : 'transfer',
      at: e.at,
      tx: e.tx,
      from: e.kind === 'minted' ? null : (e.kind === 'received' ? e.with : addr),
      to: e.kind === 'received' ? addr : (e.kind === 'minted' ? addr : e.with),
      name: e.name,
      image: e.image,
      coords: e.coords,
    }));
...
// 'offchain' when OpenSea's SignedZone stands behind this order (cancelling it there is
// binding AND free), otherwise 'onchain' (a Seaport transaction, which costs mainnet gas
// but is the only thing that truly kills an open order). Read off the order itself.
const cancelMode = params => (String(params?.zone || '').toLowerCase() === SIGNED_ZONE ? 'offchain' : 'onchain');

// The EIP-712 payload OpenSea's cancel endpoint verifies: the Seaport domain the order was
// signed under, and a one-field struct holding the order hash. Only the offerer can produce
// it, which is why the endpoint takes it in place of an API key that owns the order.
const CANCEL_TYPES = { OrderHash: [{ name: 'orderHash', type: 'bytes32' }] };

// Find one of the caller's own orders by hash — a listing or an offer, same mechanism.
async function findOwnOrder(orderHash, maker) {
  ...
  // Seaport itself only lets the offerer cancel, and OpenSea only accepts the offerer's
  // signature. Re-check here so neither is ever the first line of defence.
...
// `mode: 'onchain'` forces the paid route. The client uses it as the fallback when a free
// cancellation is refused, so a member is never left unable to withdraw an order.
async function prepareCancel({ orderHash, maker, mode }) {
  if (!configured()) fail('not_configured', 503);
  const { found, p } = await findOwnOrder(orderHash, maker);

  if (mode !== 'onchain' && cancelMode(p) === 'offchain') {
    return {
      mode: 'offchain',
      protocolAddress: found.protocol_address || SEAPORT,
      typedData: {
        types: { EIP712Domain: EIP712_DOMAIN_FIELDS, ...CANCEL_TYPES },
        domain: seaportDomain(), primaryType: 'OrderHash',
        message: { orderHash: String(found.order_hash) },
      },
    };
  }
  ...
  return { mode: 'onchain', transactions: [{ purpose: 'CANCEL', to: SEAPORT, data, value: '0x0' }], chainId: '0x1' };
}

// Hand the offerer's signature to OpenSea. Their orderbook stops serving the order and,
// because the SignedZone gates fulfilment, their signer stops endorsing it — so this is a
// real cancellation, not a hidden listing. No transaction, no gas.
async function submitOffchainCancel({ orderHash, maker, signature, protocolAddress }) {
  if (!configured()) fail('not_configured', 503);
  const { p } = await findOwnOrder(orderHash, maker);   // re-check ownership on the write
  if (cancelMode(p) !== 'offchain') fail('onchain_only', 409);
  const proto = /^0x[0-9a-f]{40}$/i.test(String(protocolAddress || '')) ? String(protocolAddress) : SEAPORT;
  let body;
  try {
    body = await osFetch(`/orders/chain/ethereum/protocol/${proto}/${String(orderHash)}/cancel`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ offererSignature: signature }),
    });
  } catch (err) {
    // OpenSea answers a hash it no longer has with "Some orders were not found" (osFetch
    // already turns that into not_found), and a signature it can't tie to the maker with
    // "API key account does not match the expected maker". The second is what the client
    // falls back to the paid route on, so it must not arrive as a generic outage.
    if (err.code === 'not_found') fail('not_active', 409);
    if (/does not match|signature/i.test(String(err.log || ''))) fail('bad_signature', 400);
    throw err;
  }
  return { cancelled: true, validUntil: body?.last_signature_issued_valid_until || null };
}

// Everything the house book (lib/land-book.js) needs to speak the same Seaport as this
// module: the same contracts, the same ABI fragments, the same reads. Exported as one bag
// rather than copied, so there is exactly one definition of each and the two books can
// never drift apart. The dependency runs one way only — land-book requires land-market,
// never the reverse — and every merge of the two happens up in server.js.
const internals = {
  ...
  ethCall, readOwnerOf, isApproved, readCounter, approvalAction, ETH_RPC_URL,
  landCurrency, landCurrencyByItem, unitsToAmt, wei2eth, joinMeta, fail,
  get ethers() { return ethers; },
};

module.exports = {
  internals, bookEnabled, getFees,
  ...
  createListing, myListings, myOffers, myHistory,
  walletTransfers, collectionSales, collectionMints, warmSalesArchive,
  prepareCancel, submitOffchainCancel, LAND_CONTRACT, stolen,
  currency: landCurrency, listingCurrency: landListingCurrency,
  offerCurrency: landOfferCurrency, offerCreateCurrency: landOfferCreateCurrency,
};
