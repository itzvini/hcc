...
// LISTINGS are narrower than the table above: OpenSea settles an Ethereum listing in ETH or
// WETH only and rejects every other payment asset, USDC included —
//   400 "Payment asset 0xa0b8…eb48 is not supported for listings on this chain"
// It rejects it at CREATE, after the seller has already signed, so a USDC listing passed
// every local check and then died with a wallet signature spent for nothing (hit in prod
// 2026-09-07). So listings are ETH-only and we say so before anyone signs. USDC stays open
// for OFFERS (LAND_OFFER_CURRENCIES) — OpenSea does settle those in ERC-20s — and reading
// stays wide (landCurrencyBySymbol), so a USDC listing made elsewhere would still display.
