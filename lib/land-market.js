...
const COLL_SALES_PAGES_MAX = 6; // ~300 recent sales — plenty to filter down by trait
async function collectionSales() {
  ...
  do {
    ...
    for (const e of (body.asset_events || [])) {
      ...
      const at = Number.isFinite(e.event_timestamp) ? new Date(e.event_timestamp * 1000).toISOString() : null;
      if (!tokenId || !at || !Number.isFinite(amt) || amt <= 0) continue;
      const name = e.nft?.name || `Highrise LAND #${tokenId}`;
      sales.push({
        tokenId: String(tokenId), currency: cur.code, priceAmt: amt,
        priceEth: cur.code === 'eth' ? amt : null, // USDC's ETH-equivalent added per-sale in shapeSalesHistory
        at,
    ...
    cursor = body.next || null;
  } while (cursor && ++pages < COLL_SALES_PAGES_MAX);
...
module.exports = {
  ...
  createListing, myListings, myOffers, myHistory, collectionSales, prepareCancel, LAND_CONTRACT,
