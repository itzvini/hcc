...
// Every SDK call goes through the shared gate, which paces this whole process against
// Immutable's per-IP rate limit — see lib/imx-gate.js. It matters most here: the SDK
// reports a 429 as "Generic Error" with no status on it, so a rate-limited read is
// indistinguishable from a real outage by the time it reaches us. Not being refused in
// the first place is the only reliable fix.
//
// Retries stay for genuine flakiness, but their backoff is now sized for a per-SECOND
// limit. It used to be 0.3s then 0.6s: against a one-second window, all three attempts
// landed inside the window that had just rejected us, so a rate-limited call burned its
// whole retry budget without ever waiting long enough to succeed.
const RETRY_BACKOFF_MS = [1200, 2400];

async function withRetry(label, fn, tries = 3, opts = {}) {
  for (let attempt = 1; ; attempt++) {
    try {
      return await gate.run(fn, opts);
    } catch (err) {
      if (attempt >= tries || !isTransient(err)) throw err;
      const delay = RETRY_BACKOFF_MS[Math.min(attempt - 1, RETRY_BACKOFF_MS.length - 1)] + Math.floor(Math.random() * 300);
...
async function listCollectionOffers({ nftContract, sellContract, ethContract, currency, accountAddress, lane }) {
  if (!orderbook) throw Object.assign(new Error('orderbook unavailable'), { code: 'unavailable', statusCode: 503 });
  const cur = currency || { code: 'eth', decimals: 18 };
  try {
...
// The WHOLE active bid book for a collection in one currency, no token or account filter.
// This is what makes "offers on the things you own" cheap: the club's active book is a few
// hundred orders across a handful of bidders, so one sweep (two or three pages) answers the
// question for every holder at once. The alternative — one listTokenOffers per Creature a
// wallet holds — is hundreds of upstream calls per dashboard load, and Immutable caps us at
// 5 requests a second per IP.
//
// Bounded on purpose: if the book ever outgrows BID_SWEEP_PAGES_MAX the sweep stops and says
// so through `truncated`, rather than paging forever behind a user's page load.
const BID_SWEEP_PAGE_SIZE = 200;
const BID_SWEEP_PAGES_MAX = 8; // 1,600 orders — roughly five times today's book

async function listAllBids({ nftContract, sellContract, ethContract, currency, lane = 'background' }) {
  if (!orderbook) throw Object.assign(new Error('orderbook unavailable'), { code: 'unavailable', statusCode: 503 });
  const cur = currency || { code: 'eth', decimals: 18 };
  const offers = [];
  let pageCursor, pages = 0, truncated = false;
  try {
    do {
      const res = await withRetry(`listAllBids(${cur.code}.${pages})`, () => orderbook.listBids({
        status: 'ACTIVE', buyItemContractAddress: nftContract, sellItemContractAddress: sellContract || ethContract,
        sortBy: 'created_at', sortDirection: 'desc', pageSize: BID_SWEEP_PAGE_SIZE,
        ...(pageCursor ? { pageCursor } : {}),
      }), 3, { lane });
      for (const b of (res.result || [])) offers.push(shapeOffer(b, false, cur));
      pageCursor = res.page?.next_cursor || null;
      if (pageCursor && ++pages >= BID_SWEEP_PAGES_MAX) { truncated = true; break; }
    } while (pageCursor);
    return { offers, truncated };
  } catch (err) { fail(`listAllBids(${cur.code})`, err); }
}
...
module.exports = {
  ...
  prepareOffer, createOffer, listTokenOffers, listCollectionOffers, listMyOffers, listAllBids,
};
