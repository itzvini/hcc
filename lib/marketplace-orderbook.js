...
async function prepareFulfill(orderId, takerAddress, tokenId, amountToFill) {
  ...
  // A successful fulfilment ALWAYS yields an on-chain transaction (buying a listing and
  // accepting a bid both transfer an NFT). An empty list means the orderbook answered 200
  // with a degraded / incomplete body — observed during Immutable API hiccups, where
  // fulfillOrder resolves instead of throwing. Surface it as a retryable error so the
  // client shows a real message and it leaves a server-side trace, rather than returning
  // nothing and letting the UI fall through to a generic "unavailable".
...
async function listMyOffers({ nftContract, ethContract, accountAddress }) {
  ...
  try {
    const [bids, coll] = await Promise.all([
      withRetry('listMyOffers.bids', () => orderbook.listBids({ status: 'ACTIVE', buyItemContractAddress: nftContract, sellItemContractAddress: ethContract, accountAddress, sortBy: 'created_at', sortDirection: 'desc', pageSize: 25 })),
      withRetry('listMyOffers.coll', () => orderbook.listCollectionBids({ status: 'ACTIVE', buyItemContractAddress: nftContract, sellItemContractAddress: ethContract, accountAddress, sortBy: 'created_at', sortDirection: 'desc', pageSize: 25 })),
    ]);
    return {
      offers: [
