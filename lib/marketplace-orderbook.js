...
// Unsigned transactions to buy a listing: usually [APPROVAL?, FULFILL_ORDER].
// The approval (payment-token allowance for Seaport) must be mined before the
// fulfilment is sent, or the fulfilment reverts — the client sequences that.
async function prepareBuy(listingId, takerAddress) {
  if (!orderbook) throw Object.assign(new Error('orderbook unavailable'), { code: 'unavailable', statusCode: 503 });
  let res;
  try {
    res = await orderbook.fulfillOrder(listingId, takerAddress, []);
  } catch (err) { fail(`prepareBuy(${listingId})`, err); }
...
module.exports = { available, prepareBuy, prepareSell, createSell, prepareCancel, submitCancel, loadError };
