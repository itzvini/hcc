...
async function prepareFulfill(orderId, takerAddress, tokenId) {
  if (!orderbook) throw Object.assign(new Error('orderbook unavailable'), { code: 'unavailable', statusCode: 503 });
  let res;
  try {
    res = await orderbook.fulfillOrder(orderId, takerAddress, [], undefined, tokenId != null ? String(tokenId) : undefined);
...
const prepareBuy = (listingId, takerAddress) => prepareFulfill(listingId, takerAddress, undefined);
...
// Shape one SDK bid into the public offer row the client renders. Gross = what the
// bidder pays; net = what the seller receives after the order's embedded fees
// (royalty/protocol). Never exposes more than is already public on the orderbook.
function shapeOffer(b, collection) {
  const gross = BigInt(b.sell?.[0]?.amount || '0');
  const feesWei = (b.fees || []).reduce((s, f) => s + (f.amount ? BigInt(f.amount) : 0n), 0n);
