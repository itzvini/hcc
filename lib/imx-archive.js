...
// One wallet's own StarkEx transaction log. The collection-wide `latestTrades` above only
// knows trades; this is the feed that also carries the moves nobody paid for — a gift, a
// wallet consolidation, the original 2021 mint. It spans EVERY collection the wallet ever
// touched on Immutable X, so the caller filters by contract, and `txnType` narrows it at
// the source: without it the reply also carries deposits and withdrawals, which are a
// token crossing between L1 and the rollup rather than changing hands.
const WALLET_TXNS_QUERY = `query($a:String!,$l:Int,$n:String,$t:String){
  listTransactionsV2(address:$a, limit:$l, nextToken:$n, txnType:$t){
    nextToken
    items {
      txn_id
      txn_time
      txn_type
      transfers { from_address to_address token { type token_address token_id } }
    }
  }
}`;
...
/**
 * One wallet's pre-migration Creature TRANSFERS — the moves that were never a sale.
 *
 * The sales history already reaches back into the StarkEx years; without this a wallet
 * searched there shows its trades from 2021 but its transfers only from the July 2025
 * migration onward, so a Creature that was gifted in 2022 reads as though the wallet's
 * first contact with it was the migration mint.
 *
 * `mint` is its own transaction type here, not a transfer from the zero address the way
 * zkEVM reports it, so both are asked for and the shape is normalised to match. Deposits
 * and withdrawals are left out: a token crossing between L1 and the rollup has not changed
 * hands, and showing it as one more row in a provenance list would say it did.
 *
 * Wallet-scoped and cheap — a page or two for all but the busiest addresses — so unlike the
 * collection sweep this is not held for the life of the process; the caller caches it.
 * Resolves to [] rather than throwing: the archive is an extension of the live feed, never
 * a precondition for it.
 */
const WALLET_TXNS_PAGE = 200;
const WALLET_TXNS_MAX_PAGES = 12;   // ~2,400 moves; a runaway guard, not a real ceiling

async function walletTransfers(wallet) {
  const addr = String(wallet || '').toLowerCase();
  if (!/^0x[0-9a-f]{40}$/.test(addr)) return [];
  const rows = [];
  const seen = new Set();
  for (const kind of ['transfer', 'mint']) {
    let cursor = null, pages = 0;
    do {
      let page;
      try {
        page = (await gql(WALLET_TXNS_QUERY, { a: addr, l: WALLET_TXNS_PAGE, n: cursor, t: kind }))?.listTransactionsV2;
      } catch (err) {
        console.error(`IMX archive wallet ${kind}s failed:`, err.message);
        return rows;            // keep whatever landed; the live era still answers
      }
      if (!page) break;
      for (const tx of page.items || []) {
        const at = Number(tx.txn_time);
        if (!Number.isFinite(at) || at <= 0) continue;
        for (const leg of tx.transfers || []) {
          const tok = leg.token || {};
          if (tok.type !== 'ERC721') continue;
          if (String(tok.token_address || '').toLowerCase() !== STARKEX_CREATURE_CONTRACT) continue;
          const tokenId = String(tok.token_id || '');
          if (!tokenId) continue;
          // One rollup transaction can move several Creatures, so the id alone is not a key.
          const key = `${tx.txn_id}:${tokenId}`;
          if (seen.has(key)) continue;
          seen.add(key);
          const from = String(leg.from_address || '').toLowerCase() || null;
          rows.push({
            tokenId,
            event: kind === 'mint' ? 'mint' : 'transfer',
            at: new Date(at).toISOString(),
            from: kind === 'mint' ? null : from,
            to: String(leg.to_address || '').toLowerCase() || null,
            tx: null,             // StarkEx had no per-move hash — immutascan is where it resolves
            txnId: tx.txn_id != null ? String(tx.txn_id) : null,
            era: 'imx',
          });
        }
      }
      cursor = page.nextToken || null;
    } while (cursor && ++pages < WALLET_TXNS_MAX_PAGES);
  }
...
module.exports = { getArchiveSales, getArchiveMetrics, archiveRates, walletTransfers, STARKEX_CREATURE_CONTRACT };
