...
//      - 1–4 assets   → 2 seats   (open to smaller holders)
//      - 5–14 assets  → 1 seat
//      - 15+ assets   → 1 seat
//    Brackets gate running, not voting — every eligible holder votes on all 4 races.

// `id`s are stable internal tokens (persisted on applicant/application rows, used for
// CSS accent theming and i18n label lookup) — the human labels live in locales
// (apply.tier.* / apply.bracket.*: Member / Patron / Icon). The ranges below are
// authoritative; the ids are intentionally left as-is to avoid churning stored data.
const BRACKETS = [
  { id: 'whale',  min: 15,  max: Infinity, seats: 1 },
  { id: 'mid',    min: 5,   max: 14,       seats: 1 },
  { id: 'single', min: 1,   max: 4,        seats: 2 },
];
