...
// --- Funding: the same solver, run the other way ---------------------------------------
//
// Getting ETH ONTO Immutable zkEVM, from Ethereum or from an L2 the member already holds
// funds on. This exists because the Squid route we shipped first is the wrong tool for the
// job in both directions of the trade-off. Measured 2026-08-31 on the same corridor:
//
//                     Squid                       Layerswap
//   Ethereum -> zkEVM $0.11, ~17 minutes          $0.009, ~7 seconds
//   Base     -> zkEVM $2.41 on $300, $17 on $1k   $0.42 on $300, $1.28 on $1k
//
// Squid takes Ethereum through Immutable's canonical bridge, which is 1:1 but slow; from an
// L2 it has to SWAP into zkEVM ETH, and that book is thin enough to cost 1.7% of a $1,000
// move. Layerswap pays out of its own float, so the cost barely moves with size.
//
// The custody trade-off is exactly the one documented at the top of this file, and it is
// smaller here: on the way in the member has not earned the money on our marketplace yet,
// and the amounts are typically one purchase.
//
// WHAT IS DIFFERENT FROM THE CASH-OUT, and why the validation below is not shared: on zkEVM
// the member sends an ERC-20 (`transfer` calldata to a token contract we pin). Here ETH is
// the NATIVE coin, so the deposit action is a plain value send to a Layerswap EOA with a
// 32-byte attribution memo attached. That means real money leaves the wallet to an address
// this file cannot verify, carrying bytes that would invoke the recipient if it were a
// contract. So the memo is length-checked to exactly 32 bytes: that is the line between an
// attribution tag and a call nobody has read.

// Source networks we will fund from. An allowlist, not a passthrough: `chainId` is asserted
// against what the API echoes back, so a renamed or repointed network fails closed rather
// than sending a member's ETH somewhere we did not mean.
const FUND_SOURCES = {
  ethereum: { ls: 'ETHEREUM_MAINNET', chainId: '1', chainHex: '0x1', label: 'Ethereum' },
  base: { ls: 'BASE_MAINNET', chainId: '8453', chainHex: '0x2105', label: 'Base' },
  arbitrum: { ls: 'ARBITRUM_MAINNET', chainId: '42161', chainHex: '0xa4b1', label: 'Arbitrum One' },
  optimism: { ls: 'OPTIMISM_MAINNET', chainId: '10', chainHex: '0xa', label: 'OP Mainnet' },
};
const ZK_NETWORK = 'IMMUTABLEZK_MAINNET';
// Its own switch, separate from the cash-out's. The two directions can fail independently,
// and pulling the newer one must not strand anybody trying to get their money out.
const FUND_ENABLED = process.env.LAYERSWAP_FUND_ENABLED !== '0';

const fundConfigured = () => ENABLED && FUND_ENABLED;
const srcOf = key => FUND_SOURCES[String(key || '').toLowerCase()] || null;

/** The picker's options. Safe to expose: names and chain ids, nothing else. */
const fundSources = () =>
  Object.entries(FUND_SOURCES).map(([key, s]) => ({ key, label: s.label, chainHex: s.chainHex }));

/** Smallest and largest move this corridor will take right now, in wei. */
async function fundLimits(key) {
  if (!fundConfigured()) fail('not_configured', 503);
  const src = srcOf(key);
  if (!src) fail('bad_source', 400);
  const d = await call(`/limits?source_network=${src.ls}&source_token=${TOKEN}`
    + `&destination_network=${ZK_NETWORK}&destination_token=${TOKEN}`);
  return {
    minWei: amountStrToWei(String(d.min_amount ?? '0')).toString(),
    maxWei: amountStrToWei(String(d.max_amount ?? '0')).toString(),
    minUsd: Number(d.min_amount_in_usd) || null,
...
/** Price a move in without committing to one. Cheap and unauthenticated, like the cash-out. */
async function quoteTopup(fromWei, key) {
  if (!fundConfigured()) fail('not_configured', 503);
  const src = srcOf(key);
  if (!src) fail('bad_source', 400);
  const amount = weiToAmountStr(fromWei);
  if (amountStrToWei(amount) === 0n) fail('no_route', 400, 'amount rounds to zero at 8dp');
  const d = await call(`/quote?source_network=${src.ls}&source_token=${TOKEN}`
    + `&destination_network=${ZK_NETWORK}&destination_token=${TOKEN}`
    + `&amount=${amount}&use_deposit_address=false`);
  const q = d.quote || d;
  if (!q || q.receive_amount == null) fail('unavailable', 503, 'quote had no receive_amount');
  return { ...shapeQuote(q, amount), source: key };
}

/**
 * Create the funding swap and return the exact NATIVE transaction to sign.
 *
 * Every check below guards a way real money can go missing, so none of them is optional:
 * the recipient decides who gets it, the amount decides how much, the chain id decides
 * which network it leaves from, and the memo length decides whether the bytes riding along
 * are a tag or a contract call.
 */
async function createTopup(fromWei, address, key) {
  if (!fundConfigured()) fail('not_configured', 503);
  const src = srcOf(key);
  if (!src) fail('bad_source', 400);
  const amount = weiToAmountStr(fromWei);
  ...
  const d = await call('/swaps', {
    ...
    timeout: 25000,
    body: {
      source_network: src.ls,
      source_token: TOKEN,
      destination_network: ZK_NETWORK,
      destination_token: TOKEN,
      amount: Number(amount),
      source_address: address,
      destination_address: address,
      // A failed swap refunds to the same wallet on the source chain, which is where the
      // ETH started. Without this their refund lifecycle has nowhere to send it.
      refund_address: address,
      use_deposit_address: false,
    },
  });
  ...
  const swapId = d.swap?.id;
  const action = (d.deposit_actions || [])[0];
  if (!swapId || !action?.to_address) fail('unavailable', 503, 'swap response missing id or deposit action');
  if (String(action.type) !== 'transfer') fail('unavailable', 503, `unexpected action type ${action.type}`);

  // It must be the NATIVE coin. A contract address here means Layerswap wants an ERC-20
  // move, and signing a value-carrying send against that assumption would strand the money.
  if (action.token && action.token.contract != null) {
    fail('unavailable', 503, `deposit token is not native (contract ${action.token.contract})`);
  }
  // ...and it must leave from the chain the member picked, not whichever one the API names.
  const echoed = String(action.network?.chain_id ?? '');
  if (echoed !== src.chainId) fail('unavailable', 503, `deposit action is on chain ${echoed}, expected ${src.chainId}`);

  // Recipient: a clean 20-byte address, not the burn hole, and not the member themselves
  // (which would be a no-op that burns gas and credits no swap).
  const to = String(action.to_address).toLowerCase();
  if (!/^0x[0-9a-f]{40}$/.test(to)) fail('unavailable', 503, 'deposit recipient is not an address');
  if (to === '0x' + '0'.repeat(40)) fail('unavailable', 503, 'deposit recipient is the zero address');
  if (to === String(address).toLowerCase()) fail('unavailable', 503, 'deposit recipient is the sender');

  // Amount: EXACTLY what we asked to move. Not more, which overspends; not less, which
  // under-funds the swap and strands it.
  const wantWei = amountStrToWei(amount);
  let gotWei;
  try { gotWei = BigInt(String(action.amount_in_base_units)); } catch { gotWei = null; }
  if (gotWei !== wantWei) fail('unavailable', 503, `deposit amount ${gotWei} does not match the requested ${wantWei}`);

  // The memo. Empty is fine; exactly 32 bytes is what Layerswap sends. Anything longer is a
  // call this file has not read, riding on a transaction that carries value.
  const data = action.call_data == null ? '0x' : String(action.call_data);
  if (!/^0x([0-9a-f]{64})?$/i.test(data)) fail('unavailable', 503, 'deposit call_data is not an empty or 32-byte memo');
  ...
  return {
    ...
    source: key,
    swapId,
    fromWei: wantWei.toString(),
    chainHex: src.chainHex,
    recipient: to,
    tx: {
      to,
      value: '0x' + wantWei.toString(16),   // a native send: the money rides in `value`
      data,                                  // 32-byte attribution memo, forwarded verbatim
    },
  };
}

module.exports = {
  configured, quoteCashout, createCashout, getStatus, limits, weiToAmountStr,
  fundConfigured, fundSources, fundLimits, quoteTopup, createTopup,
};
