...
async function getAxelarStatus(txHash, { reverse = false } = {}) {
  ...
  // A message that ran out of relay gas is recoverable — addNativeGas on the source chain's
  // gas service tops it up — but nobody finds that unaided, and two cash-outs left underfunded
  // on 2026-07-31 were still approved and unexecuted nineteen days later. So the stall is
  // reported, and the client points at the place that can fix it.
  //
  // We deliberately do NOT hand back a log index for the client to build that call itself.
  // Axelar exposes several similarly-named indices (call.logIndex, call._logIndex,
  // call.receipt.logIndex) that are scoped differently — one counts ContractCall events, one
  // counts every log in the transaction — and addNativeGas silently accepts a wrong one. Since
  // that call carries real IMX, and we have no stalled message to verify the semantics
  // against, guessing would risk spending a member's money on the wrong event. Axelarscan's
  // own page for the transaction offers the top-up and gets the index right, so that is where
  // the client sends them until someone can confirm the correct field against a live stall.
  return {
    stage,
    destUrl: destTx ? destBase + destTx : null,
    srcTxHash: d.call?.transactionHash || null,
  };
}
