...
async function fetchRoute(fromAmountWei, address) {
...
// Squid quotes are exact-input, so start ~1% over and refine once with the observed rate.
async function quoteBridge(neededWei, address) {
  if (!configured()) fail('not_configured', 503);

  let fromAmount = (neededWei * 101n) / 100n;
  let { route, requestId, quoteId } = await fetchRoute(fromAmount, address);
  let toAmount = BigInt(route.estimate.toAmount);
  if (toAmount < neededWei) {
    // Scale by the shortfall ratio plus a 0.5% cushion, then re-quote.
    fromAmount = (fromAmount * neededWei * 1005n) / (toAmount * 1000n);
    ({ route, requestId, quoteId } = await fetchRoute(fromAmount, address));
...
module.exports = { configured, quoteBridge, getStatus };
