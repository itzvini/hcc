...
//   /api/auth/dev-login?user=Icon&creatures=15           → 15+ bracket (Icon)
//   /api/auth/dev-login?creatures=6                      → 5–14 bracket (Patron)
//   /api/auth/dev-login?creatures=2                      → 1–4 bracket (Member)
...
module.exports = async function devLogin(request, response, url) {
  ...
  const serverName = q.get('server'); // simulate the Highrise Discord display name (ballot name)
  const profile = {
    id: `dev-${username}`, username: `${username} (DEV)`, avatar: null,
    serverName: serverName || null, inGuild: !!serverName,
    highriseName: q.get('hrname') || null,
    highriseIcon: q.get('icon') || null, // simulate the Highrise avatar pic
  };
  const sid = await db.createSession(profile.id, profile, eligibility);

  response.writeHead(302, {
    Location: '/council/vote',
