...
// This lookup sits on the sign-in critical path, so it gets ONE retry: on a rate limit
// (429), and on a transport failure — a timeout/abort or a dropped connection to
// webapi.highrise.game, which throws from fetch() rather than returning a status. Those
// are usually transient blips upstream, and a single one shouldn't hard-fail the whole
// login (observed in the audit log as callback errors at this stage). The two retry
// paths share one budget, so a request is attempted at most twice.
async function fetchHighriseWallet(discordId, { retried = false } = {}) {
  let res;
  try {
    res = await fetch(`${HIGHRISE_API}/discord/users/${encodeURIComponent(discordId)}/wallet`, {
      headers: { 'X-Api-Key': HIGHRISE_KEY },
      signal: AbortSignal.timeout(15000),
    });
  } catch (err) {
    if (!retried) {
      await new Promise(r => setTimeout(r, 500)); // transient timeout/network blip — brief pause, then retry once
      return fetchHighriseWallet(discordId, { retried: true });
    }
    throw err; // second attempt also failed — let the caller surface it
  }
