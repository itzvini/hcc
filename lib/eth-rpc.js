...
/**
 * Every transfer matching `params`, newest first, following Alchemy's pageKey.
 *
 * Alchemy-ONLY, deliberately. `alchemy_getAssetTransfers` is a vendor method that no other
 * provider in the failover list implements, so there is nothing to fail over to: the two
 * public nodes would answer "unknown method", which arrives as an rpcError (a real answer)
 * and would be rethrown anyway. Callers must treat an absent key as "this history is
 * unavailable" and degrade, never as "there are no transfers" — the same rule the market
 * feeds follow. `alchemyEnabled` says which it is BEFORE you ask.
 *
 * Why it's worth the single-provider dependency: the alternative is eth_getLogs, which the
 * free tier caps at a 10-block range (see NO_ALCHEMY_METHODS) and would need tens of
 * thousands of calls to sweep this chain's history. This does it in one call per 1,000.
 */
async function assetTransfers(params, { maxPages = 5, timeoutMs = 20000 } = {}) {
  if (!ALCHEMY) throw Object.assign(new Error('alchemy_getAssetTransfers: no ALCHEMY_API_KEY'), { unavailable: true });
  const out = [];
  let pageKey, pages = 0;
  do {
    const res = await callOne(ALCHEMY, 'alchemy_getAssetTransfers',
      [{ maxCount: '0x3e8', ...params, ...(pageKey ? { pageKey } : {}) }], timeoutMs);
...
module.exports = {
  ethRpc, ethCallVia, providerNames, assetTransfers,
