...
async function init() {
  ...
  await pool.query(`
    DO $$ BEGIN
      IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'gas_asset_cooldowns')
         AND NOT EXISTS (SELECT 1 FROM gas_asset_cooldowns) THEN
        DROP TABLE gas_asset_cooldowns;
      END IF;
    END $$;
  `);
