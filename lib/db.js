...
const mem = {
  ...
  gasLocks: new Map(),       // creature token_id -> { grant_id, wallet, locked_at } — permanent, one claim per Creature
...
async function init() {
  ...
  // Per-Creature lock, permanent. The three identity checks alone would let a member with a
  // big bag claim, move the Creatures to a second wallet and a second Discord account, and
  // claim again. So a grant locks EVERY Creature the paid wallet held at that moment, for
  // good, and a claim needs at least one Creature that has never been locked. Splitting or
  // selling the bag then buys nothing: the lock follows the token, not the holder.
  await pool.query(`
...
//   assets_used   — every Creature this wallet holds has already been used for a claim
//
...
async function reserveGasGrant({ discordId, highriseId = null, wallet, tokenIds = [], dailyCap }) {
  const w = String(wallet || '').trim().toLowerCase();
  ...
  if (usePg) {
    ...
    try {
      ...
      // At least one held Creature must never have been used for a claim.
      if (ids.length) {
        const { rows: [{ n: locked }] } = await client.query(
          `SELECT COUNT(*)::int AS n FROM gas_asset_locks WHERE token_id = ANY($1::text[])`, [ids]);
      ...
      const { rows: [grant] } = await client.query(
        ...
        [discordId, hr, w]);
      if (ids.length) {
        // DO NOTHING, not DO UPDATE: a token's lock belongs to the grant that first spent
        // it, and re-pointing it at a later grant would let releaseGasGrant delete a lock
        // that an earlier, real payout is still relying on.
        await client.query(
          `INSERT INTO gas_asset_locks (token_id, grant_id, wallet)
  ...
  if (ids.length && ids.every(id => mem.gasLocks.has(id))) return { reason: 'assets_used' };
  ...
  for (const t of ids) if (!mem.gasLocks.has(t)) mem.gasLocks.set(t, { grant_id: id, wallet: w, locked_at: now });
  return { id };
}
...
async function gasGrantStatus({ discordId, highriseId = null, wallet, tokenIds = [] }) {
    ...
    const [{ rows: prior }, { rows: [{ n: today }] }, { rows: [{ n: locked }] }] = await Promise.all([
      ...
      pool.query(
        `SELECT COUNT(*)::int AS n FROM gas_grants
         WHERE status <> 'failed' AND reserved_at > now() - interval '24 hours'`),
      ids.length
        ? pool.query(`SELECT COUNT(*)::int AS n FROM gas_asset_locks WHERE token_id = ANY($1::text[])`, [ids])
  ...
  return {
    used: p ? (p.discord_id === discordId ? 'account_used' : (hr && p.highrise_id === hr) ? 'highrise_used' : 'wallet_used') : null,
    assetsUsed: ids.length > 0 && ids.every(id => mem.gasLocks.has(id)),
