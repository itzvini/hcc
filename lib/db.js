...
async function upsertAnnouncement(row) {
  const {
    ...
    authorAvatar = null, content = '', attachments = [], embeds = [], postedAt, editedAt = null,
  } = row;
    ...
    await pool.query(
      `INSERT INTO announcements
         (message_id, channel_id, author_id, author_name, author_display, author_avatar,
          content, attachments, embeds, posted_at, edited_at, updated_at)
      ...
      [String(messageId), String(channelId), authorId, authorName, authorDisplay, authorAvatar,
       content, JSON.stringify(attachments || []), JSON.stringify(embeds || []),
  ...
  if (usePg) {
    const { rows } = await pool.query(
      `SELECT message_id, channel_id, author_id, author_name, author_display, author_avatar,
              content, attachments, embeds, posted_at, edited_at
       FROM announcements
