...
const mem = {
  ...
  gasGrants: [],             // gas-faucet payout ledger — { id, discord_id, wallet, amount_wei, tx_hash, status, reserved_at }
  gasCooldowns: new Map(),   // creature token_id -> { grant_id, wallet, cooled_until } (ms)
...
async function init() {
  ...
  // BEFORE the transaction is signed: it reserves the member's cooldown, and a failed
  // send deletes it again (releaseGasGrant) so a broken RPC never costs someone a month.
  ...
  // Per-Creature cooldown. The account and wallet cooldowns alone would let a member with
  // a big bag claim, move the Creatures to a second wallet + Discord account, and claim
  // again. So a grant cools EVERY Creature the wallet holds at that moment, and a claim
  // needs at least one Creature that isn't cooling. Splitting the bag then buys nothing.
  await pool.query(`
    CREATE TABLE IF NOT EXISTS gas_asset_cooldowns (
  ...
  `);
  await pool.query(`CREATE INDEX IF NOT EXISTS gas_asset_cooldowns_until_idx ON gas_asset_cooldowns (cooled_until);`);
...
// Reserve a grant, or explain why not. Returns { id } on success, else
// { reason, retryAfterMs } where reason is one of:
//   daily_cap        — the site-wide safety cap for today is used up
//   account_cooldown — this Discord account claimed too recently
//   wallet_cooldown  — this wallet was paid too recently (a second Discord account)
//   asset_cooldown   — every Creature this wallet holds is still cooling
//
// All four checks and the insert happen inside ONE transaction under advisory locks on
// the account and the wallet, so two clicks (or two tabs) can't both pass the gate.
// The caller MUST follow up with settleGasGrant or releaseGasGrant.
async function reserveGasGrant({ discordId, wallet, tokenIds = [], cooldownMs, dailyCap }) {
  const w = String(wallet || '').trim().toLowerCase();
  ...
  if (usePg) {
    ...
    try {
      ...
      // Most recent non-failed grant for this account, and for this wallet.
      const { rows: recent } = await client.query(
        `SELECT discord_id, wallet, reserved_at FROM gas_grants
         WHERE status <> 'failed' AND (discord_id = $1 OR wallet = $2)
           AND reserved_at > now() - $3::interval
         ORDER BY reserved_at DESC LIMIT 1`,
        [discordId, w, `${Math.ceil(cooldownMs / 1000)} seconds`]);
      if (recent.length) {
        const r = recent[0];
        const retryAfterMs = Math.max(0, new Date(r.reserved_at).getTime() + cooldownMs - Date.now());
        await client.query('ROLLBACK');
        return { reason: r.discord_id === discordId ? 'account_cooldown' : 'wallet_cooldown', retryAfterMs };
      }

      // At least one held Creature must be off cooldown.
      if (ids.length) {
        const { rows: cooling } = await client.query(
          `SELECT token_id, cooled_until FROM gas_asset_cooldowns
           WHERE token_id = ANY($1::text[]) AND cooled_until > now()`, [ids]);
        if (cooling.length >= ids.length) {
          const soonest = Math.min(...cooling.map(c => new Date(c.cooled_until).getTime()));
          await client.query('ROLLBACK');
          return { reason: 'asset_cooldown', retryAfterMs: Math.max(0, soonest - Date.now()) };
        }
      }

      const { rows: [grant] } = await client.query(
        `INSERT INTO gas_grants (discord_id, wallet) VALUES ($1, $2) RETURNING id`, [discordId, w]);
      if (ids.length) {
        await client.query(
          `INSERT INTO gas_asset_cooldowns (token_id, grant_id, wallet, cooled_until)
           SELECT t, $2, $3, $4 FROM unnest($1::text[]) AS t
           ON CONFLICT (token_id) DO UPDATE
             SET grant_id = EXCLUDED.grant_id, wallet = EXCLUDED.wallet, cooled_until = EXCLUDED.cooled_until`,
          [ids, grant.id, w, until]);
      }
      await client.query('COMMIT');
  ...
  const recent = live
    .filter(g => (g.discord_id === discordId || g.wallet === w) && now - g.reserved_at < cooldownMs)
    .sort((a, b) => b.reserved_at - a.reserved_at)[0];
  if (recent) {
    return {
      reason: recent.discord_id === discordId ? 'account_cooldown' : 'wallet_cooldown',
      retryAfterMs: Math.max(0, recent.reserved_at + cooldownMs - now),
    };
  }
  if (ids.length) {
    const cooling = ids.map(id => mem.gasCooldowns.get(id)).filter(c => c && c.cooled_until > now);
    if (cooling.length >= ids.length) {
      return { reason: 'asset_cooldown', retryAfterMs: Math.max(0, Math.min(...cooling.map(c => c.cooled_until)) - now) };
    }
  }
  const id = String(++memGrantId);
  mem.gasGrants.push({ id, discord_id: discordId, wallet: w, amount_wei: '0', tx_hash: null, status: 'pending', reserved_at: now });
  for (const t of ids) mem.gasCooldowns.set(t, { grant_id: id, wallet: w, cooled_until: until.getTime() });
  return { id };
}
...
// The send failed. Drop the reservation and the cooldowns it set, so an RPC blip doesn't
// cost the member their next month. Keeps nothing: a grant that never paid isn't a payout.
async function releaseGasGrant(id) {
  if (usePg) {
    await pool.query(`DELETE FROM gas_asset_cooldowns WHERE grant_id = $1`, [id]);
    await pool.query(`DELETE FROM gas_grants WHERE id = $1 AND status = 'pending'`, [id]);
    return;
  }
  for (const [t, c] of mem.gasCooldowns) if (c.grant_id === String(id)) mem.gasCooldowns.delete(t);
  const i = mem.gasGrants.findIndex(x => x.id === String(id) && x.status === 'pending');
  if (i !== -1) mem.gasGrants.splice(i, 1);
}

// Read-only view for the UI gate: when did this account/wallet last get paid, and how
// much of today's cap is left. No reservation, no locks — safe to call on every render.
async function gasGrantStatus({ discordId, wallet, cooldownMs }) {
  const w = String(wallet || '').trim().toLowerCase();
  if (usePg) {
    const [{ rows: recent }, { rows: [{ n: today }] }] = await Promise.all([
      pool.query(
        `SELECT reserved_at FROM gas_grants
         WHERE status <> 'failed' AND (discord_id = $1 OR wallet = $2)
         ORDER BY reserved_at DESC LIMIT 1`, [discordId, w]),
      pool.query(
        `SELECT COUNT(*)::int AS n FROM gas_grants
         WHERE status <> 'failed' AND reserved_at > now() - interval '24 hours'`),
    ]);
    const lastAt = recent.length ? new Date(recent[0].reserved_at).getTime() : null;
    return { lastAt, nextAt: lastAt == null ? null : lastAt + cooldownMs, grantsToday: today };
  }
  const now = Date.now();
  const live = mem.gasGrants.filter(g => g.status !== 'failed');
  const last = live
    .filter(g => g.discord_id === discordId || g.wallet === w)
    .sort((a, b) => b.reserved_at - a.reserved_at)[0];
  return {
    lastAt: last ? last.reserved_at : null,
    nextAt: last ? last.reserved_at + cooldownMs : null,
