...
// 0.05 IMX (~$0.008): roughly 12 buys or 80 transfers, with headroom for a bad block.
//
// Cost ceiling: at 0.05 IMX per wallet, paying every one of the ~5,300 Creature-holding
// wallets that exists comes to ~265 IMX, about $42. Cost is not the risk here. The risks
...
//   • It pays ONLY the wallet the member's Highrise account is linked to, never an
//     address from the request body. There is no "enter a wallet" path to abuse.
...
// 0.041 and not a full 0.05.
const TARGET_WEI = imxToWei(process.env.GAS_FAUCET_TARGET_IMX, 5n * 10n ** 16n);   // 0.05 IMX
...
const COOLDOWN_DAYS = intEnv('GAS_FAUCET_COOLDOWN_DAYS', 30);
const COOLDOWN_MS = COOLDOWN_DAYS * 24 * 60 * 60 * 1000;
// Site-wide circuit breaker. At the default target this caps a runaway day at 10 IMX
// (~$1.60), which is the point: it bounds the damage from a bug we haven't thought of.
...
// Pay a member's gas.
//
// `wallet` MUST already be proven to belong to the caller (the Highrise-linked wallet on
// their session) and `tokenIds` must be the Creatures that wallet actually holds — both
// established by the caller against the chain, not the request body.
//
// Returns { ok: true, txHash, amountWei } or { ok: false, reason, retryAfterMs? }.
// Reasons: disabled, has_gas, blocked, screen_unavailable, faucet_empty, send_failed,
// plus the cooldown/cap reasons from db.reserveGasGrant.
async function grant({ db, discordId, wallet, tokenIds, walletImxWei }) {
  ...
  const reserved = await db.reserveGasGrant({
    discordId, wallet, tokenIds, cooldownMs: COOLDOWN_MS, dailyCap: DAILY_CAP,
  });
  if (!reserved.id) return { ok: false, reason: reserved.reason, retryAfterMs: reserved.retryAfterMs };
  ...
  try {
    ...
    db.recordEvent({
      ...
      detail: { wallet, amountWei: String(amountWei), txHash, screen: screened.source },
    });
...
module.exports = {
  ...
  COOLDOWN_MS, COOLDOWN_DAYS, DAILY_CAP, TARGET_WEI, TRIGGER_WEI,
};
