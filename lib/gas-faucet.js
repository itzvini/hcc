...
//     tainted for 30 days, and the Discord account and Highrise account behind the session
//     each get exactly one claim, for good. Swapping wallets buys nothing, and neither does
//     shuffling the Creatures inside that window, because the taint follows the token. The
//     taint expires so a later buyer never inherits a claim they didn't make.
...
// How long a claim taints the Creatures that were in the paid wallet. NOT for life: a
// Creature outlives its owner's claim, and marking it permanently would hand whoever buys
// it next a dead claim they never used. 30 days makes bag-splitting pointless without
// damaging the asset.
const TAINT_DAYS = intEnv('GAS_FAUCET_TAINT_DAYS', 30);
const TAINT_MS = TAINT_DAYS * 24 * 60 * 60 * 1000;
...
// TAINT_DAYS is deliberately NOT here. The UI says a claim puts your Creatures out of
// action for a time; it doesn't say how long, and this payload is the only place the
// number could leak to a browser.
const policy = () => ({
...
// of those Creatures is then tainted for TAINT_DAYS, which is what stops the wallet being
...
async function grant({ db, discordId, highriseId, wallet, tokenIds, walletImxWei }) {
  ...
  // An unreachable oracle refuses a grant to somebody who qualifies, so it has to leave a
  // mark. It didn't, and that cost us: on 2026-09-09 the screen's single RPC node 429'd
  // every call for days and the only evidence in the ledger was an absence of grants.
  // A console line nobody was tailing is not an audit trail.
  if (!screened.ok) {
    db.recordEvent({ event: 'gas.screen_unavailable', discordId, ok: false, detail: { wallet, source: screened.source } });
  ...
  const reserved = await db.reserveGasGrant({
    discordId, highriseId, wallet, tokenIds, taintMs: TAINT_MS, dailyCap: DAILY_CAP,
  });
...
module.exports = {
  ...
  DAILY_CAP, TARGET_WEI, TRIGGER_WEI, TAINT_DAYS,
};
